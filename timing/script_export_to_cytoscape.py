...
def export_cytoscape(timing):
    ...
    lines = ['{} {} {}'.format(x.r1, safe_pathway_name(x.pathway), x.r2) for x in res.res]
    save_file(join(results_dir(), 'cytoscape', 'regions.sif'), lines)
    lines = ['{} ({}) {} = {}'.format(x.r1, safe_pathway_name(x.pathway), x.r2, edge_weight(x.pval)) for x in res.res]
